...
def directory_name(filename):
	return os.path.join(identifier_directory, filename)
...
more_threshold_filename = os.path.join(identifier_directory, "more_threshold.png")
less_threshold_filename = directory_name("less_threshold.png")
print('starting...')
...
# Clean the dataframe
df.columns = ['TIME', 'CH1', 'CH2', 'CH3', 'CH4', 'NA', 'TIME_MATH1', 'MATH1']
# df = df[['TIME', 'MATH1']].dropna()

# Convert the TIME and MATH1 columns to numeric
df['TIME'] = pd.to_numeric(df['TIME'])
df['MATH1'] = pd.to_numeric(df['MATH1'])

# Calculate B using the formula B = MATH1 / 48800
df['B'] = df['MATH1'] / 48800
df['CalcNorm'] = np.sqrt((df['CH1']**2) + (df['CH2']**2) + (df['CH3']**2))
...
def save_histogram(use_df, col_name, filename):
	use_df[col_name].hist(bins=50)
...
save_histogram(df, 'MATH1', voltage_hist_filename)
save_histogram(df, 'B', b_hist_filename)
...
threshold = np.float64(0.15e-6)
# Filter the DataFrame to get rows where B < 0.15 * 10^-6
less_filtered_df = df[df['B'] < threshold]
more_filtered_df = df[df['B'] > threshold]
save_histogram(less_filtered_df, 'TIME', less_threshold_filename)
save_histogram(more_filtered_df, 'TIME', more_threshold_filename)
save_histogram(less_filtered_df, 'CH4', directory_name('ch4_less.png'))
save_histogram(more_filtered_df, 'CH4', directory_name('ch4_more.png'))
...
with open(results_path, 'w') as f:
    ...
    f.write('\n\n')
    f.write('less_filtered: \n')
    f.write(less_filtered_df["TIME"].describe().to_string())
    f.write('more_filtered: \n')
    f.write(more_filtered_df["TIME"].describe().to_string())
    # f.write('TIMES where B > 0.15 * 10^-6: \n')
    # f.write(more_filtered_df['TIME'].to_string()) 
